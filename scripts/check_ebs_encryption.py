...
def scan_manifests(repo_root: Path, default_enabled: bool | None, live_scs: dict[str, dict[str, Any]]) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
    manifest_scs = {}
    findings = []
    ...
    for path in yaml_files:
        ...
        for doc in load_yaml_documents(path):
            ...
            if kind == "StorageClass":
                sc = manifest_scs.get(name)
                if sc and sc["isEbs"] and sc.get("encrypted") != "true":
                    findings.append(
                        {
                            ...
                            "message": f"EBS StorageClass encrypted={sc.get('encrypted') or '<missing>'}",
                            "fix": 'parameters.encrypted: "true"를 추가하세요.',
                        }
                    )
                elif sc and sc["isEbs"]:
                    findings.append(
                        {
                            "risk": "ok",
                            "file": str(path),
                            "line": find_line(path, "parameters:") or find_line(path, "provisioner:"),
                            "resource": f"StorageClass/{name}",
                            "message": 'EBS StorageClass에 encrypted: "true"가 명시되어 있습니다.',
                            "fix": "수정 불필요",
                        }
                    )
    ...
    return list(manifest_scs.values()), findings
        ...
        findings.append(
            {
                ...
                "fix": "--cluster 옵션으로 live StorageClass를 조회하거나 StorageClass YAML을 포함하세요.",
            }
        )
        return
...
def main() -> None:
    ...
    manifest_scs, findings = scan_manifests(repo_root, default["enabled"], live_scs)
            ...
            json.dumps(
                {
                    ...
                    "manifestStorageClasses": manifest_scs,
                    "findings": findings,
