...
run "plan_deploys_core_addons" {
  ...
  assert {
    condition     = kubernetes_storage_class_v1.encrypted_gp3.allow_volume_expansion == true
    error_message = "The encrypted StorageClass must allow safe workload volume expansion."
  }
